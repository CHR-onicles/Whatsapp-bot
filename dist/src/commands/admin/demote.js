...
const execute = (client, msg) => __awaiter(void 0, void 0, void 0, function* () {
    if ((yield (0, misc_1.getMutedStatus)()) === true)
        return;
    const userToDemote = (0, helpers_1.extractCommandArgs)(msg)[0];
    ...
    if (foundUser) {
        ...
        if (foundUser.id.user === client.info.wid.user) {
            yield msg.reply((0, helpers_1.pickRandomReply)(data_1.DEMOTE_BOT_REPLIES));
            return;
        }
        else if (foundUser.id.user === process.env.GRANDMASTER) {
            yield msg.reply((0, helpers_1.pickRandomReply)(data_1.DEMOTE_GRANDMASTER_REPLIES));
            return;
        }
        const isFoundUserBotAdmin = yield (0, helpers_1.isUserBotAdmin)(foundUser);
        if (isFoundUserBotAdmin) {
            yield (0, misc_1.removeBotAdmin)(foundUser.id.user);
            yield msg.reply("Bot admin dismissed successfully! ✅"); //todo: Add more replies for this later
            return;
        }
        else {
            yield msg.reply("This user is not a bot admin 🤦🏽‍♂️"); // todo: Add more replies for this later
        }
    }
    else {
...
module.exports = {
    ...
    alias: ["dem"],
    category: "admin",
    help: `To use this command, type:\n*${helpers_1.currentPrefix}demote @user*`,
    execute,
};

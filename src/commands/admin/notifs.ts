...
import {
  ...
  enableOrDisableNotificationForCourse,
} from "../../models/misc";
...
const execute = async (client: IClient, msg: Message) => {
  ...
  const helper = async (courseCode: string, enableStatus: boolean) => {
    ...
    await msg.reply(
      `Notifications for *${
        courses[courseCode as keyof typeof courses]
      }* have been turned *${enableStatus ? "ON" : "OFF"}*`
    );
  };
